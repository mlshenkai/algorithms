    ...
    def forward(self, feat, img_metas=None):
        ...
        if valid_ratios is not None:
            ...
            for i in range(valid_ratios.size(0)):
                valid_step = (
                    torch.minimum(
                        torch.tensor(T), torch.ceil(T * valid_ratios[i]).int()
                    )
                    - 1
                )
        ...
        if valid_ratios is not None:
            ...
            for i in range(valid_ratios.size(0)):
                valid_width = torch.minimum(
                    torch.tensor(w), torch.ceil(w * valid_ratios[i]).int()
                )
                if valid_width < w:
    ...
    def forward(self, feat, data=None):
        ...
        if self.training:
            ...
            final_out = self.decoder(
                feat, holistic_feat, label=None, img_metas=data, train_mode=False
            )
