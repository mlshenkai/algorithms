...
from algorithm.ocr.text_ocr.modeling.common import Activation
import numpy as np
...
class GridGenerator(nn.Module):
    ...
    def build_P_paddle(self, I_r_size):
        ...
        I_r_grid_x = (torch.arange(-I_r_width, I_r_width, 2) + 1.0) / torch.tensor(
            np.array([I_r_width])
        )

        I_r_grid_y = (torch.arange(-I_r_height, I_r_height, 2) + 1.0) / torch.tensor(
            np.array([I_r_height])
        )
    ...
    def build_inv_delta_C_paddle(self, C):
        ...
        delta_C = torch.cat(  # F+3 x F+3
            [
                ...
                torch.concat(
                    [torch.zeros((2, 3)), C.transpose(0, 1)], dim=1
                ),  # 2 x F+3
                torch.concat(
                    [torch.zeros((1, 3)), torch.ones((1, F))], dim=1
                ),  # 1 x F+3
            ],
            axis=0,
        )
