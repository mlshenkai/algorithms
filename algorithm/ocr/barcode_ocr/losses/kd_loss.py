...
class KdLoss(nn.Module):
    def __init__(
        self, alpha=1.0, beta=10, ohem_ratio=3, reduction="mean", eps=1e-6, k=50
    ):
        """
        Implement PSE Loss.
    ...
    def forward(self, pred, teacher_outputs, batch):
        ...
        teacher_outputs_binary_maps = self.step_function(
            teacher_outputs[:, 0, :, :], teacher_outputs[:, 1, :, :]
        )
        loss_shrink_maps = self.bce_loss(
            shrink_maps, teacher_outputs[:, 0, :, :], batch["shrink_mask"]
        )
        loss_threshold_maps = self.l1_loss(
            threshold_maps, teacher_outputs[:, 1, :, :], batch["threshold_mask"]
        )
        # metrics = dict(loss_shrink_maps=loss_shrink_maps, loss_threshold_maps=loss_threshold_maps)
        metrics = dict()

        loss_binary_maps = self.dice_loss(
            binary_maps, teacher_outputs_binary_maps, batch["shrink_mask"]
        )
        loss_all = (
